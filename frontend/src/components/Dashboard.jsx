...
import {pushSwitchesAtom, toggleSwitchesAtom, sliderSwitchesAtom, colorPickerSwitchesAtom } from '../store/switchAtoms'
import { PushButton, ToggleSwitch, Gauge, ColorPicker, Slider, Widget } from './ControlElements';

export const Dashboard = () => {
  const deviceStatus = useRecoilValue(deviceStatusAtom);
      ...
      <section className="h-full flex flex-col bg-gray-800 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 gap-3 overflow-y-auto">
          {deviceStatus.map((device) => (
            <div key={device.deviceId} className="bg-gray-700 h-auto p-3 rounded-lg flex items-center">
              ...
              <div className="text-right space-y-1.5">
                <div className="flex items-center justify-end gap-2">
                  ...
                  <div className={`w-3 h-3 rounded-full ${device.isOnline ? 'bg-green-500' : 'bg-red-500'}`} />
                </div>
                <p className="text-sm font-medium mb-1 text-gray-400">
                  Signal Strength: <span className="text-sm font-medium mb-1 text-white">{device.strength}%</span>
                </p>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="overflow-y-auto custom-scrollbar h-screen bg-gray-900 text-white p-6 pb-10 mb-10 flex flex-col">
      <header className="mb-8">
        ...
        <p className="text-white/80 mt-2">Real-time device monitoring and control</p>
      </header>
