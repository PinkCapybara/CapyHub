...
import { createGauge, editGauge, deleteElements } from '../../services/api/endpoints';
...
export const Gauges = () => {
  ...
  const { mutate: createMutate, loading: createLoading } = useApiMutation(createGauge);
  const { mutate: editMutate, loading: editLoading } = useApiMutation(editGauge);
              ...
              >
                <div className="flex flex-col flex-1">
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-200">{gauge.name}</span>
                    <span className="text-sm text-gray-400 font-mono">ID: {gauge._id}</span>
                  </div>
      ...
      <div className="w-96 sticky top-6 h-fit">
        <div className="bg-gray-800 p-6 border border-gray-700 rounded-lg shadow-lg">
          <h2 className="text-xl font-bold mb-4 text-gray-100 dark:text-white">
            {isEditing ? 'Edit Gauge' : isDeleting ? 'Delete Gauge' : 'Add New Gauge'}
          </h2>
